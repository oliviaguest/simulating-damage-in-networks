#ifndef _lib_cairox_2_0_h_
#define _lib_cairox_2_0_h_
...
typedef enum {
    MARK_NONE, MARK_TRIANGLE_OPEN, MARK_TRIANGLE_FILLED, MARK_DIAMOND_OPEN, MARK_DIAMOND_FILLED, MARK_SQUARE_OPEN, MARK_SQUARE_FILLED, MARK_CIRCLE_OPEN, MARK_CIRCLE_FILLED
} MarkerType;

typedef struct cairox_font_properties {
    char       *face;
    int         size;
    PangoStyle  style;
    PangoWeight weight;
...
typedef struct cairox_scale_parameters {
    GtkPositionType pt;
    double w;
    double h;
    double max;
...
extern void cairox_paint_marker(cairo_t *cr, double x, double y, MarkerType mark);
...
extern void cairox_centre_text(cairo_t *cr, double x0, double y0, PangoLayout *layout, char *label);
extern void cairox_vector_draw(cairo_t *cr, double x0, double y0, Boolean colour, int nf, double *vals);

extern void cairox_scale_parameters_set(CairoxScaleParameters *ss, GtkPositionType pt, double w, double h, double max, double min, int ticks, char *fmt);
extern void cairox_scale_draw(cairo_t *cr, PangoLayout *layout, CairoxScaleParameters *ss, double x0, double y0, Boolean colour);

extern void cairox_pixel_matrix_draw(cairo_t *cr, double x, double y, double dx, double dy, double pixels[], int n, Boolean colour);

extern CairoxFontProperties *font_properties_create(char *face, int size, PangoStyle style, PangoWeight weight, const char *colour);
extern void font_properties_set_size(CairoxFontProperties *fp, int size);
extern void font_properties_destroy(CairoxFontProperties *fp);
extern void pangox_layout_set_font_properties(PangoLayout *layout, CairoxFontProperties *fp);
