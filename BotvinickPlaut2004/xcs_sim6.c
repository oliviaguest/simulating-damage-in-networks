...
static TaskType sim6_task = {TASK_NONE, DAMAGE_NONE, {TRUE, FALSE, FALSE, FALSE, FALSE}};
...
static int cs_sim6_results[3][10][NUM_VARIANTS];
...
static Boolean cs_sim6_viewer_expose(GtkWidget *widget, GdkEvent *event, void *data)
{
...
static void initialise_cs_sim6_results()
{
    int i, j, l;
    for (j = 0; j < 3; j++) {
        for (l = 0; l < 10; l++) {
            for (i = 0; i < NUM_VARIANTS; i++) {
                cs_sim6_results[j][l][i] = 0;
            }
        }
    }
}
...
static void generate_results_for_n_runs(int l, long count)
{
    ...
    for (i = 0; i < count; i++) {
        for (tt = 0; tt < 3; tt++) {
            sim6_task.base = (BaseTaskType) tt;
            initialise_state(&sim6_task);
            s = run_one_simulation();
            cs_sim6_results[tt][l][s]++;
        }
        cs_sim6_viewer_expose(NULL, NULL, NULL);
    }
}

static void generate_cs_sim6_results_callback(GtkWidget *mi, void *count)
{
    /* Here we iterate over ten weight files with the specified initial  */
    /* contents of the mug.                                              */
...
static void clear_cs_sim6_results_callback(GtkWidget *mi, void *dummy)
{
    initialise_cs_sim6_results();
    if (viewer_widget != NULL) {
        cs_sim6_viewer_expose(NULL, NULL, NULL);
    }
}

static void callback_toggle_coffee_present(GtkWidget *button, TaskType *task)
{
    task->initial_state.mug_contains_coffee = (Boolean) (GTK_TOGGLE_BUTTON(button)->active);
}

static void callback_toggle_tea_present(GtkWidget *button, TaskType *task)
{
    task->initial_state.mug_contains_tea = (Boolean) (GTK_TOGGLE_BUTTON(button)->active);
}

static void callback_toggle_cream_present(GtkWidget *button, TaskType *task)
{
    task->initial_state.mug_contains_cream = (Boolean) (GTK_TOGGLE_BUTTON(button)->active);
}

static void callback_toggle_sugar_present(GtkWidget *button, TaskType *task)
{
    task->initial_state.mug_contains_sugar = (Boolean) (GTK_TOGGLE_BUTTON(button)->active);
}

void create_cs_sim6_table(GtkWidget *vbox)
{
    ...
    gtk_toolbar_append_item(GTK_TOOLBAR(toolbar), "Reset", NULL, NULL, NULL, G_CALLBACK(clear_cs_sim6_results_callback), NULL);
    gtk_toolbar_append_item(GTK_TOOLBAR(toolbar), "10 Trials", NULL, NULL, NULL, G_CALLBACK(generate_cs_sim6_results_callback), (void *)10);
    gtk_toolbar_append_item(GTK_TOOLBAR(toolbar), "100 Trials", NULL, NULL, NULL, G_CALLBACK(generate_cs_sim6_results_callback), (void *)100);

    tmp = gtk_vseparator_new();
    gtk_box_pack_start(GTK_BOX(hbox), tmp, FALSE, FALSE, 0);
    gtk_widget_show(tmp);

    /*--- The check buttons: Coffee ---*/

    /*--- Fiddle with the mug contents: ---*/
    tmp = gtk_label_new("   Initial mug contents:");
    gtk_box_pack_start(GTK_BOX(hbox), tmp, FALSE, FALSE, 0);
    gtk_widget_show(tmp);
    /*--- The check buttons: Coffee ---*/
    tmp = gtk_label_new("   Coffee:");
    gtk_box_pack_start(GTK_BOX(hbox), tmp, FALSE, FALSE, 0);
    gtk_widget_show(tmp);
    tmp = gtk_check_button_new();
    gtk_box_pack_start(GTK_BOX(hbox), tmp, FALSE, FALSE, 0);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(tmp), FALSE);
    g_signal_connect(G_OBJECT(tmp), "toggled", G_CALLBACK(callback_toggle_coffee_present), &sim6_task);
    gtk_widget_show(tmp);
    /*--- The check buttons: Tea ---*/
    tmp = gtk_label_new("   Tea:");
    gtk_box_pack_start(GTK_BOX(hbox), tmp, FALSE, FALSE, 0);
    gtk_widget_show(tmp);
    tmp = gtk_check_button_new();
    gtk_box_pack_start(GTK_BOX(hbox), tmp, FALSE, FALSE, 0);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(tmp), FALSE);
    g_signal_connect(G_OBJECT(tmp), "toggled", G_CALLBACK(callback_toggle_tea_present), &sim6_task);
    gtk_widget_show(tmp);
    /*--- The check buttons: Sugar ---*/
    tmp = gtk_label_new("   Sugar:");
    gtk_box_pack_start(GTK_BOX(hbox), tmp, FALSE, FALSE, 0);
    gtk_widget_show(tmp);
    tmp = gtk_check_button_new();
    gtk_box_pack_start(GTK_BOX(hbox), tmp, FALSE, FALSE, 0);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(tmp), FALSE);
    g_signal_connect(G_OBJECT(tmp), "toggled", G_CALLBACK(callback_toggle_sugar_present), &sim6_task);
    gtk_widget_show(tmp);
    /*--- The check buttons: Cream ---*/
    tmp = gtk_label_new("   Cream:");
    gtk_box_pack_start(GTK_BOX(hbox), tmp, FALSE, FALSE, 0);
    gtk_widget_show(tmp);
    tmp = gtk_check_button_new();
    gtk_box_pack_start(GTK_BOX(hbox), tmp, FALSE, FALSE, 0);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(tmp), FALSE);
    g_signal_connect(G_OBJECT(tmp), "toggled", G_CALLBACK(callback_toggle_cream_present), &sim6_task);
    gtk_widget_show(tmp);

    /* The viewer: */

    viewer_widget = gtk_drawing_area_new();
    g_signal_connect(G_OBJECT(viewer_widget), "expose_event", G_CALLBACK(cs_sim6_viewer_expose), NULL);
