...
/* The results matrix has 4 dimensions: instruction, bowl state, weights, sequence */
static int cs_sim4_results[3][2][10][NUM_VARIANTS];

/* abc is the simulation sub-number: 3a, 3b or 3c: */
static char abc = 'a';

/******************************************************************************/

static double mean_result(int j, int k, int i)
{
    double sum = 0;
    int l = 0;

    for (l = 0; l < 10; l++) {
        sum = sum + cs_sim4_results[j][k][l][i];
    }
    return(sum / (double) l);
}
...
static void load_weight_file(int l)
{
    ...
    g_snprintf(file, 64, "DATA_SIMULATION_4/weights_3%c_%d.dat", abc, l);
...
static Boolean cs_sim4_viewer_expose(GtkWidget *widg, GdkEvent *event, void *data)
{
    if (!GTK_WIDGET_MAPPED(viewer_widget) || (viewer_surface == NULL)) {
        ...
        g_snprintf(buffer, 128, "SUGAR BOWL STATE:"); y = y + 15;
        cairox_text_parameters_set(&tp, 10, y, PANGOX_XALIGN_LEFT, PANGOX_YALIGN_BOTTOM, 0.0);
        cairox_paint_pango_text(cr, &tp, layout, buffer);
        ...
        for (j = 0; j < 3; j++) {
            ...
            cairox_paint_line(cr, &lp, 340+100*j-w/2-1, y+2, 340+100*j+w/2+1, y+2);

            for (k = 0; k < 2; k++) {
                double count = 0.0;

                y = 30;
                g_snprintf(buffer, 128, (k == 0 ? "open" : "closed"));
                cairox_text_parameters_set(&tp, 360-40*k+100*j, y, PANGOX_XALIGN_CENTER, PANGOX_YALIGN_BOTTOM, 0.0);
                cairox_paint_pango_text(cr, &tp, layout, buffer);
                w = pangox_layout_get_string_width(layout, buffer);
                cairox_paint_line(cr, &lp, 360-40*k+100*j-w/2-1, y+2, 360-40*k+100*j+w/2+1, y+2);

                y = y + 5;
                for (i = 0; i < NUM_VARIANTS; i++) {
                    double mean = mean_result(j, k, i);
                    g_snprintf(buffer, 128, "%4.1f", mean); y = y + 15;
                    cairox_text_parameters_set(&tp, 375-40*k+100*j, y, PANGOX_XALIGN_RIGHT, PANGOX_YALIGN_BOTTOM, 0.0);
                    cairox_paint_pango_text(cr, &tp, layout, buffer);
                    count += mean;
                }

                y = y + 2;
                cairox_paint_line(cr, &lp, 345-40*k+100*j, y, 377-40*k+100*j, y);
...
static void initialise_cs_sim4_results()
{
    int i, j, k, l;
    for (j = 0; j < 3; j++) {
        for (k = 0; k < 2; k++) {
...
static void generate_results_for_n_runs(int bowl_state, int l, long count)
{
    int tt, i, s;

    for (i = 0; i < count; i++) {
        for (tt = 0; tt < 3; tt++) {
            TaskType task = {(BaseTaskType) tt, DAMAGE_NONE, {bowl_state, FALSE, FALSE, FALSE, FALSE}};

            initialise_state(&task);
            s = run_one_simulation();
            cs_sim4_results[tt][bowl_state][l][s]++;
        }
        cs_sim4_viewer_expose(NULL, NULL, NULL);
    }
}

static void generate_cs_sim4_results_callback(GtkWidget *mi, void *count)
{
    ...
    for (l = 0; l < 10; l++) {
        load_weight_file(l);
        generate_results_for_n_runs(0, l, (long) count);
        generate_results_for_n_runs(1, l, (long) count);
    }
}

static void clear_cs_sim4_results_callback(GtkWidget *mi, void *dummy)
{
    initialise_cs_sim4_results();
    if (viewer_widget != NULL) {
        cs_sim4_viewer_expose(NULL, NULL, NULL);
    }
}

static void cs_sim4_results_set_task_callback(GtkWidget *tb, void *choice)
{
    if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(tb))) {
        abc = (char) ((long) choice);
        clear_cs_sim4_results_callback(tb, choice);
    }
}

void create_cs_sim4_viewer(GtkWidget *vbox)
{
    ...
    gtk_toolbar_append_item(GTK_TOOLBAR(toolbar), "Reset", NULL, NULL, NULL, G_CALLBACK(clear_cs_sim4_results_callback), NULL);
    gtk_toolbar_append_item(GTK_TOOLBAR(toolbar), "10 Trials", NULL, NULL, NULL, G_CALLBACK(generate_cs_sim4_results_callback), (void *)10);
    gtk_toolbar_append_item(GTK_TOOLBAR(toolbar), "100 Trials", NULL, NULL, NULL, G_CALLBACK(generate_cs_sim4_results_callback), (void *)100);

    /**** Widgets for setting the task: ****/

    tmp = gtk_radio_button_new_with_label(NULL, "C");
    g_signal_connect(G_OBJECT(tmp), "toggled", G_CALLBACK(cs_sim4_results_set_task_callback), (void *)'c');
    ...
    g_signal_connect(G_OBJECT(tmp), "toggled", G_CALLBACK(cs_sim4_results_set_task_callback), (void *)'b');
    ...
    g_signal_connect(G_OBJECT(tmp), "toggled", G_CALLBACK(cs_sim4_results_set_task_callback), (void *)'a');
    ...
    g_signal_connect(G_OBJECT(viewer_widget), "expose_event", G_CALLBACK(cs_sim4_viewer_expose), NULL);
