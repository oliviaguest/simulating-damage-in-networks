...
Version 2.0:
18/04/17: Upgraded gdk_colour -> gdk_rgba (for GDK3)

Version 2.0 patch #1:
28/02/19: Fix minor bug in drawing arrows with curved vertices
...
void cairox_paint_marker(cairo_t *cr, double x, double y, MarkerType mark)
{
    double d = 4.0; // Size should probably be an input parameter
...
CairoxFontProperties *font_properties_create(char *face, int size, PangoStyle style, PangoWeight weight, const char *colour)
{
...
#else
        gdk_rgba_parse(&text_colour, colour);
        fp->colour = gdk_rgba_copy(&text_colour);
#endif
    }
    return(fp);
}
