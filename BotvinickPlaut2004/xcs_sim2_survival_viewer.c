...
static int sv_data[MAX_STEPS];
static double sv_level = 0.000;

static char *sv_label[3] = {
    "Activation Noise (s.d.)",
    "Weight Noise (s.d.)",
    "Connections Severed (proportion)"
};

#define TASK_STEPS ((sv_task.base == TASK_COFFEE) ? 37 : 20)
...
static void sv_run_and_score_weight_noise(Network *net, TaskType *task)
{
    double    *vector_in;
    double    *vector_out;
    ActionType this[MAX_STEPS];
...
static void sv_run_and_score_weight_lesion(Network *net, TaskType *task)
{
    double    *vector_in;
    double    *vector_out;
    ActionType this[MAX_STEPS];
...
static void survival_graph_write_to_cairo(cairo_t *cr, int width, int height)
{
    ...
    if (sv_data[0] == 1) {
        g_snprintf(buffer, 128, "Survival plot for %d trial; %s = %5.3f", sv_data[0], sv_label[sv_task.damage-1], sv_level);
    }
    else {
        g_snprintf(buffer, 128, "Survival plot for %d trials; %s = %5.3f", sv_data[0], sv_label[sv_task.damage-1], sv_level);
    }
    ...
    g_snprintf(buffer, 128, "Step in %s task", (sv_task.base == TASK_COFFEE ? "coffee" : "tea"));
    ms = 5 * (1 + (TASK_STEPS/5)); // Maximum steps to show on the horizontal axis
    graph_set_axis_properties(survival_graph, GTK_ORIENTATION_HORIZONTAL, 0, ms, 1+(ms/5), "%2.0f", buffer);
    graph_set_dataset_properties(survival_graph, 0, "%% Correct", 0.0, 0.0, 0.0, 0, LS_SOLID, MARK_DIAMOND_FILLED);

    graph_set_legend_properties(survival_graph, FALSE, 0.0, 0.0, NULL);

    if (sv_data[0] > 0) {
        for (i = 0; i < TASK_STEPS; i++) {
            survival_graph->dataset[0].x[i] = i;
            survival_graph->dataset[0].y[i] = 100 * sv_data[i] / (double) sv_data[0];
        }
        survival_graph->dataset[0].points = TASK_STEPS;
    }
    else {
        survival_graph->dataset[0].points = 0;
    }

    cairox_draw_graph(cr, survival_graph, TRUE);
}
...
static void survival_viewer_step_callback(GtkWidget *mi, void *count)
{
    ...
    while (j-- > 0) {
        ...
        else if (sv_task.damage == DAMAGE_WEIGHT_NOISE) {
            Network *tmp_net = network_copy(xg.net);
            sv_run_and_score_weight_noise(tmp_net, &sv_task);
            network_tell_destroy(tmp_net);
        }
        else if (sv_task.damage == DAMAGE_WEIGHT_LESION) {
            Network *tmp_net = network_copy(xg.net);
            sv_run_and_score_weight_lesion(tmp_net, &sv_task);
            network_tell_destroy(tmp_net);
        }
    }
    survival_viewer_expose(NULL, NULL, NULL);
...
void create_bp_sim2_survival_plot_viewer(GtkWidget *vbox)
{
    ...
    gtk_widget_show(tmp);
    /*--- The spin button: ---*/
    adj = (GtkAdjustment *)gtk_adjustment_new(sv_level, 0.0, 1.0, 0.001, 0.10, 0);
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(tmp), (sv_task.base == TASK_COFFEE));
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(tmp), (sv_task.base == TASK_TEA));
